...
bool Wad::InstallContents(const char *installpath)
{
	...
	for (cnt = 0; cnt < tmd_data->num_contents; cnt++)
	{
		...
		if(content->type == 0x8001) {
			...
			// private content
			snprintf(filepath, sizeof(filepath), "%s/title/%08x/%08x/content/%08x.app", installpath, (unsigned int) (tmd_data->title_id >> 32),
			 (unsigned int) tmd_data->title_id, (unsigned int) content->cid);
		}
