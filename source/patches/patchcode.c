		...
		while (i < tempgameconfsize)
		{
			...
			while (i != tempgameconfsize && tempgameconf[i] != ':')
			{
				...
				//if (!autobootcheck)
				{
					...
					{
						ret = sscanf((char *) tempgameconf + i, "( %lux , %lux", &codeaddr, &codeval);
						if (ret == 2)
						{
					...
					{
						ret = sscanf((char *) (tempgameconf + i), "( %lux , %lux , %lux , %lux", &codeaddr, &codeval, &codeaddr2, &codeval2);
						if (ret == 4)
						{
							*(gameconf + (gameconfsize / 4)) = 0;
							gameconfsize += 4;
					...
					{
						ret = sscanf((char *) (tempgameconf + i), "( %lux%lu", &codeval, &tempoffset);
						if (ret == 1)
						{
							gameconfsize += 4;
							temp = 0;
							...
							*(gameconf + (gameconfsize / 4) - temp - 1) = temp;
							ret = sscanf((char *) (tempgameconf + i), " , %lux , %lux , %lux , %lux", &codeaddr, &codeaddr2, &codeoffset, &codeval2);
							if (ret == 4)
							{
								*(gameconf + (gameconfsize / 4)) = codeaddr;
								gameconfsize += 4;
